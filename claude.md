...
**Project:** IJS VAULT Admin Panel - User vault document management system (Android/iOS/Huawei app)

**Communication Style:**

- Minimal English, very brief responses
- No extra explanations unless asked
- Use @.claude/agents/research-agent.md for research

**Code Standards:**

- Files <200 lines (components), <250 (pages), <150 (hooks)
- One job per file
- No `any` types, always TypeScript
- Clean, scalable, organized code
- Refactor if file >200 lines

**Project Pattern:**

- Use custom `useRouter` hook from `@/routes/hooks` (NOT `useNavigate` from react-router-dom)
- Named import for Icon: `import { Icon } from '@/components/icon'`
- MSW for API mocking in `src/_mock/`
- Zustand for state, React Query for data fetching
- All endpoints in `src/config/api-endpoints.ts`
- Types in `src/types/`

**IJS VAULT Context:**

- Users manage personal documents in categories/subcategories
- Admin can: view users, suspend/activate/delete, monitor sessions, export data
- No password reset by admin, self-registration only
- Security-focused: session monitoring, suspicious activity alerts
